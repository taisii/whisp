...
tmp_dir="$(mktemp -d /tmp/whisp-bench.XXXXXX)"
...
for i in $(seq 1 "$runs"); do
  ./.build/debug/whisp --stt-file "$input_wav" >"$tmp_dir/rest-$i.log"
done

echo "running streaming benchmark (realtime)..."
for i in $(seq 1 "$runs"); do
  ./.build/debug/whisp --stt-stream-file "$input_wav" --chunk-ms 120 --realtime >"$tmp_dir/stream-$i.log"
done

rest_avg_total="$(awk -F': ' '/^total_ms:/{sum+=$2; n++} END{if(n>0) printf "%.1f", sum/n; else printf "n/a"}' "$tmp_dir"/rest-*.log)"
stream_avg_total="$(awk -F': ' '/^total_ms:/{sum+=$2; n++} END{if(n>0) printf "%.1f", sum/n; else printf "n/a"}' "$tmp_dir"/stream-*.log)"
stream_avg_finalize="$(awk -F': ' '/^finalize_ms:/{sum+=$2; n++} END{if(n>0) printf "%.1f", sum/n; else printf "n/a"}' "$tmp_dir"/stream-*.log)"
stream_avg_send="$(awk -F': ' '/^send_ms:/{sum+=$2; n++} END{if(n>0) printf "%.1f", sum/n; else printf "n/a"}' "$tmp_dir"/stream-*.log)"
audio_seconds="$(awk -F': ' '/^audio_seconds:/{print $2; exit}' "$tmp_dir/rest-1.log" 2>/dev/null || true)"
...
echo "rest_transcript_sample: $(awk -F': ' '/^transcript:/{print $2; exit}' "$tmp_dir/rest-1.log")"
echo "stream_transcript_sample: $(awk -F': ' '/^transcript:/{print $2; exit}' "$tmp_dir/stream-1.log")"
