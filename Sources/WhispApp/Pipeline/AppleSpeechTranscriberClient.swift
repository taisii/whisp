...
final class AppleSpeechTranscriberClient: AppleSpeechTranscriber, @unchecked Sendable {
    ...
    func finishStreaming() async throws -> (transcript: String, usage: STTUsage?) {
        guard let state = await streamingStore.finish() else {
            throw AppError.invalidArgument("Apple Speech Transcriber streaming セッションが開始されていません")
        }
    ...
    private func transcribeWithSpeechModule<Module: SpeechModule>(
        ...
        let tmpURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("whisp-apple-speech-transcriber-\(UUID().uuidString)")
        ...
        do {
            ...
            resultsTask.cancel()
            throw AppError.io("Apple Speech Transcriber 文字起こしに失敗: \(error.localizedDescription)")
        }
