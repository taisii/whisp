...
final class BenchmarkRunRecorder: @unchecked Sendable {
    ...
    init(
        ...
        self.run = BenchmarkRunRecord(
            ...
            kind: kind,
            status: .running,
            createdAt: BenchmarkExecutor.isoNow(),
            updatedAt: BenchmarkExecutor.isoNow(),
            options: options,
        ...
        try store.appendOrchestratorEvent(
            ...
            event: BenchmarkOrchestratorEvent(
                ...
                stage: .runStart,
                status: .ok,
                recordedAtMs: BenchmarkExecutor.nowEpochMs()
            )
        )
    }
    ...
    func appendCaseResult(_ result: BenchmarkCaseResult) throws {
        ...
        try store.appendOrchestratorEvent(
            ...
            event: BenchmarkOrchestratorEvent(
                ...
                status: BenchmarkExecutor.eventStatus(from: result.status),
                recordedAtMs: BenchmarkExecutor.nowEpochMs(),
                attrs: [
    ...
    func finalize(
        ...
        run.status = status
        run.updatedAt = BenchmarkExecutor.isoNow()
        try store.saveRun(run)
        try store.appendOrchestratorEvent(
            runID: run.id,
            event: BenchmarkOrchestratorEvent(
...
extension BenchmarkExecutor {
    ...
    static func loadManualBenchmarkCases(path: String) throws -> [ManualBenchmarkCase] {
        ...
        let records = try store.loadCases(path: trimmed)
        return records.map { ManualBenchmarkCase(record: $0) }
    }
