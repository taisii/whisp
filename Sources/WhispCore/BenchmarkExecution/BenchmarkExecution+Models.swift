...
struct IntentLabel: Codable {
    ...
    var canonicalJSONString: String {
        ...
        let payload = CanonicalPayload(intent: intent, slots: slots)
...
extension ManualBenchmarkCase {
    init(record: BenchmarkDatasetCaseRecord) {
        id = record.id
        runID = record.runID
        audioFile = (record.audioFile ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        accessibility = record.accessibility.map { snapshot in
            let focusedElement = snapshot.focusedElement.map { element in
                ManualBenchmarkAccessibilityFocusedElement(
                    selectedText: element.selectedText,
                    selectedRange: element.selectedRange.map {
                        ManualBenchmarkAccessibilityTextRange(location: $0.location, length: $0.length)
                    },
            ...
            return ManualBenchmarkAccessibilitySnapshot(focusedElement: focusedElement)
        }
        visionImageFile = record.visionImageFile
        visionImageMimeType = record.visionImageMimeType
        intentGold = record.intentGold.map { IntentLabel(intent: $0.intent, slots: $0.slots) }
        intentSilver = record.intentSilver.map { IntentLabel(intent: $0.intent, slots: $0.slots) }
        labelConfidence = record.labelConfidence
        labels = record.labels.map {
            ManualBenchmarkLabels(
                transcriptGold: $0.transcriptGold,
                transcriptSilver: $0.transcriptSilver,
                intentGold: $0.intentGold.map { IntentLabel(intent: $0.intent, slots: $0.slots) },
                intentSilver: $0.intentSilver.map { IntentLabel(intent: $0.intent, slots: $0.slots) },
